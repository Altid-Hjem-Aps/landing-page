...
export default function Nav({ spiirBanner = false }: NavProps) {
  ...
  function handleCTA(source: 'nav' | 'spiir-banner') {
  ...
  // Uden banner (alle sider undtagen /spiir-alternativ): PRÆCIS den oprindelige
  // struktur — fixed + transform + backdrop-filter direkte på <nav>. Forsiden
  // må ikke ændre sig, og backdrop-filter under en transformeret forælder er
  // kendt WebKit-bøvl, så den kombination undgås helt her.
  if (!spiirBanner) {
    return (
      <nav
  ...
  // Med banner (/spiir-alternativ): banner + nav glider op/ned som én enhed.
  return (
    <div
    ...
    >
      {/* Spiir-banner: tidsbegrænset kampagne (Spiir lukkede 8. juni 2026).
      ...
      <button
        type="button"
        onClick={() => handleCTA('spiir-banner')}
      ...
      >
        ...
        <span className="hidden md:inline">
          Spiir er lukket — mist ikke overblikket over dine faste udgifter.{' '}
        </span>
        <span className="md:hidden">Brugte du Spiir? </span>
