...
export default function WaitlistForm({ variant = 'light', id, defaultView = 'form', source = DEFAULT_SIGNUP_SOURCE, embedded = false, onSignup }: Props) {
  ...
  if (isDark) {
    ...
    return (
      <form id={id} onSubmit={e => { e.preventDefault(); submitStep1() }} className={darkCardClass} style={darkCardStyle}>
        ...
        {(name || email || phone) && <ConsentCheckbox dark checked={consentAll} onChange={setConsentAll} />}
  ...
  return (
    <form id={id} onSubmit={e => { e.preventDefault(); submitStep1() }}>
      ...
      {/* Consent sits just above the button, and only appears once the visitor
          has started filling in the form (name, email or phone). */}
      {(name || email || phone) && <ConsentCheckbox dark={false} checked={consentAll} onChange={setConsentAll} />}
